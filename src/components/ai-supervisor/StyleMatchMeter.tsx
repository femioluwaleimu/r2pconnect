...
import { formatPercent, toNumber } from "@/lib/numberFormat";
...
export default function StyleMatchMeter({ score, styleName = "Selected Style" }: StyleMatchMeterProps) {
  ...
  const getScoreColor = () => {
    if (normalizedScore >= 80) return "text-green-600 dark:text-green-400";
    if (normalizedScore >= 60) return "text-amber-600 dark:text-amber-400";
  ...
  const getStatusIcon = () => {
    if (normalizedScore >= 80) return <CheckCircle className="w-4 h-4 text-green-500" />;
    if (normalizedScore >= 60) return <Sparkles className="w-4 h-4 text-amber-500" />;
    return <AlertTriangle className="w-4 h-4 text-red-500" />;
  };

  const getStatusLabel = () => {
    if (normalizedScore >= 80) return "Excellent Match";
    if (normalizedScore >= 60) return "Good Match";
    if (normalizedScore >= 40) return "Fair Match";

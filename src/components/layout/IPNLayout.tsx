...
const sidebarItems = [
  { icon: LayoutDashboard, label: "Dashboard", href: "/ipn" },
  { icon: Building2, label: "Companies", href: "/ipn/companies" },
  { icon: Briefcase, label: "Opportunities", href: "/ipn/opportunities" },
  { icon: Users, label: "Applicants", href: "/ipn/applicants" },
  { icon: Wallet, label: "Revenue & Wallet", href: "/ipn/revenue" },
  { icon: BarChart3, label: "Analytics", href: "/ipn/analytics" },
  { icon: UserIcon, label: "Profile", href: "/ipn/profile" },
  { icon: Settings, label: "Settings", href: "/ipn/settings" },
];

interface IPNLayoutProps {
...
export default function IPNLayout({ children }: IPNLayoutProps) {
  ...
  useEffect(() => {
    if (user) {
      ...
      fetchNotifications();
      // Check activation
      supabase.from("ipn_activations").select("status, rejection_reason").eq("user_id", user.id).maybeSingle().then(({ data }) => {
  ...
  if (activationChecked && activationStatus !== "activated" && isProtectedPage) {
    return <Navigate to="/ipn/activate" replace />;
  }
        ...
        <div className="p-4 border-b border-border flex-shrink-0">
          <Link to="/" className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="block text-xs text-primary">IPN Portal</span>
            </div>
          </Link>
        </div>
        ...
        <div className="p-3 border-t border-border flex-shrink-0 bg-card">
          <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-accent/30 transition-colors mb-2">
            <Avatar className="w-10 h-10 shadow-md">
              ...
              <AvatarFallback className="bg-gradient-to-br from-orange-500 to-red-500 text-primary-foreground font-semibold">
        ...
        <header className="sticky top-0 z-30 bg-card/80 backdrop-blur-lg border-b border-border px-4 lg:px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg sm:text-xl font-bold text-foreground">IPN Dashboard</h1>
                <p className="text-xs sm:text-sm text-muted-foreground">Industry Partner Network</p>
              </div>
            </div>
                ...
                <DropdownMenuContent align="end" className="w-80">
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.slice(0, 5).map((notification) => (
                      <DropdownMenuItem
                        key={notification.id}
                        className={cn("flex flex-col items-start gap-1 p-3 cursor-pointer", !notification.is_read && "bg-primary/5")}
                        onClick={() => {
                      ...
                      >
                        ...
                        <span className="text-[10px] text-muted-foreground">{formatLagos(notification.created_at)}</span>
                      </DropdownMenuItem>
                    ))
                  )}
                </DropdownMenuContent>

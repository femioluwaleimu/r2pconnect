...
import {
  ...
  Percent, Shield, Zap, X, CheckCircle2
} from "lucide-react";
...
export default function IndustrySubscriptionSummary() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!planId) { navigate("/industry/subscriptions"); return; }
      ...
      if (error || !planData) {
        ...
        navigate("/industry/subscriptions");
        return;
      }
  ...
  const validateCoupon = async () => {
    ...
    try {
      ...
      if (coupon.valid_until && new Date(coupon.valid_until) < new Date()) { setCouponError("This coupon has expired"); return; }
      if (coupon.max_uses && coupon.current_uses >= coupon.max_uses) { setCouponError("This coupon has reached its usage limit"); return; }
      if (coupon.plan_id && coupon.plan_id !== plan?.plan_id) { setCouponError("This coupon doesn't apply to the selected plan"); return; }
    ...
    <IndustryLayout>
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => navigate("/industry/subscriptions")} className="rounded-xl">
        ...
        <Card className="rounded-2xl shadow-lg border-primary/20 overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-primary/10 to-accent/10 border-b">
            <div className="flex items-center justify-between">
              ...
              <Badge className="bg-primary/10 text-primary border-primary/20 text-lg px-4 py-2">{plan.period}</Badge>
            </div>
          </CardHeader>
          <CardContent className="p-6 space-y-6">
            ...
            {plan.ai_matches_per_challenge > 0 && (
              <div className="flex items-center gap-3 p-3 bg-violet-500/10 rounded-xl">
                ...
                <div>
                  <p className="font-semibold text-violet-700 dark:text-violet-400">
                    {plan.ai_matches_per_challenge} AI Matches / Challenge
                  </p>
                  <p className="text-xs text-muted-foreground">For researcher matching</p>
                </div>
              </div>
            )}
          </CardContent>
          ...
          <CardContent className="space-y-4">
            {appliedCoupon ? (
              ...
              <div className="flex gap-2">
                ...
                />
                <Button onClick={validateCoupon} disabled={!couponCode.trim() || validatingCoupon} className="rounded-xl">
          ...
          <CardContent className="p-6 space-y-4">
              ...
              {appliedCoupon && (
                <div className="flex justify-between text-stat-green">
                  <span className="flex items-center gap-1">
                    <Percent className="w-4 h-4" />Discount ({appliedCoupon.discount_percentage}%)
                  </span>
            ...
            >
              {processing ? (
                ...
                <><Zap className="w-5 h-5 mr-2" />{finalAmount <= 0 ? "Activate Plan" : `Pay ${formatCurrency(finalAmount, 'NGN')}`}</>
              )}
            </Button>
